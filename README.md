...
13. v2026.09.23.9 补全 Xray 配置生命周期：健康检查、自动回滚、恢复菜单和配置导入导出。
   1. `apply_xray_config` 支持可选重启验证；Xray 重启失败时打印 systemd/journal 诊断并自动恢复 apply 前备份。
   2. routing 新增/删除/清空、完整配置重生成、WARP 开关/重置均迁移到统一安全 apply 流程。
   3. 完整配置构建期间默认规则只修改内存，最终一次性校验/备份/apply，避免中间配置多次落盘。
   4. 配置管理新增备份列表和手动恢复；恢复前再次备份当前配置，恢复后立即验证 Xray 服务。
   5. 新增 tar.gz 配置包导出/导入，包含 Xray 配置、脚本配置和 manifest；导入前验证 JSON 与 Xray 配置。
   6. WARP reset 会刷新 Docker 容器 IP 到 Xray `warp` outbound，避免容器重建后继续引用旧 IP。
...
- 按时间查看最近的 Xray 自动备份。
- 选择一个备份恢复；当前配置会先再次备份，恢复后自动重启并验证 Xray。
- 导出配置包到 `~/.xray-script/exports/`，包含 `xray-config.json`、`script-config.json` 和 manifest。
- 导入脚本生成的配置包；导入前会校验格式和 Xray 配置，失败不会静默覆盖。

当一次需要重启验证的配置 apply 导致 Xray 无法正常启动时，脚本会自动尝试恢复 apply 前的备份，并重新启动 Xray。
